...
{
    public interface ITransPemakaianInternalService
    {
        #region Header

        Task<List<tr_pemakaian_internal>> GetAllOpenTrPemakaianInternalByParams(List<ParameterSearchModel> param);

        Task<List<tr_pemakaian_internal>> GetAllTrPemakaianInternalByParams(List<ParameterSearchModel> param);
        Task<tr_pemakaian_internal> GetTrPemakaianInternalById(long pemakaian_internal_id);

        Task<(bool, long, string)> AddTrPemakaianInternal(tr_pemakaian_internal_insert data);
        Task<(bool, long, string)> UpdateTrPemakaianInternalValidated(tr_pemakaian_internal_update_to_validated data);
        Task<(bool, long, string)> UpdateTrPemakaianInternalCanceled(tr_pemakaian_internal_update_to_canceled data);

        #endregion

        #region Detail Item

        Task<List<tr_pemakaian_internal_detail_item>> GetAllTrPemakaianInternalDetailItemByParams(List<ParameterSearchModel> param);
        Task<List<tr_pemakaian_internal_detail_item>> GetTrPemakaianInternalDetailItemByPemakaianInternalId(long pemakaian_internal_id);

        #endregion

        #region Detail Item Batch

        Task<List<tr_pemakaian_internal_detail_item_batch>> GetTrPemakaianInternalDetailItemBatchByDetailItemId(long pemakaian_internal_detail_item_id);
        Task<List<tr_pemakaian_internal_detail_item_batch>> GetTrPemakaianInternalDetailItemBatchByPemakaianInternalId(long pemakaian_internal_id);

        #endregion

        #region Detail Upload

        Task<List<tr_pemakaian_internal_detail_upload>> GetTrPemakaianInternalDetailUploadByPemakaianInternalId(long pemakaian_internal_id);
        Task<long> AddTrPemakaianInternalDetailUpload(tr_pemakaian_internal_detail_upload_insert data);
        Task<short> DeleteTrPemakaianInternalDetailUpload(long pemakaian_internal_detail_upload_id);

        #endregion
    }

    public class TransPemakaianInternalService : ITransPemakaianInternalService
    {
        private readonly SQLConn _db;
        private readonly SetupItemDao _itemDao;
        private readonly TransPemakaianInternalDao _dao;
        ...
        public TransPemakaianInternalService(SQLConn db, TransPemakaianInternalDao dao,
        ...
        public async Task<(bool, long, string)> AddTrPemakaianInternal(tr_pemakaian_internal_insert data)
        {
        ...
        public async Task<(bool, long, string)> UpdateTrPemakaianInternalValidated(tr_pemakaian_internal_update_to_validated data)
        {
                ...
                if (validateId > 0)
                {
                    ...
                    var rekapForHeader = issuedDataDetail.GroupBy(q => q.id_item)
                        .Select(res => new
                        {
                            ...
                            qty_on_hand = (data.details.FirstOrDefault().detailBatch.Count > 0 ?
                            data.details.Where(q => q.id_item == res.FirstOrDefault().id_item)
                            .FirstOrDefault()
                            .detailBatch.Sum(s => s.qty_pemakaian_internal) : 0),
                            //res.Sum(s => s.qty_pemakaian_internal),
                        ...
                        foreach (var item in rekapForHeader)
                        {
                            ...
                            if (item.qty_on_hand > 0)
                            {
                                ...
                                var kartuStokParam = new mm_kartu_stok_item_insert_pengurangan_stok
                                {
                                    ...
                                    keterangan = "KELUAR PEMAKAIAN INTERNAL",
                                    ...
                                    foreach (var detail in item.details)
                                    {
                                        ...
                                        if (detail.detailBatch.Count > 0)
                                        {
                                            foreach (var batch in detail.detailBatch)
                                            {
                                                ...
                                                #region Insert detail batch

                                                batch.pemakaian_internal_detail_item_id = detailId;
                                                batch.pemakaian_internal_id = validateId;

                                                var detailBatchId = await this._dao.AddTrPemakaianInternalDetailItemBatch(batch);

                                                if (detailBatchId <= 0)
                                                {

                                                    this._db.rollBackTrans();
                                                    return (false, 0, "Gagal input pemakaian internal detail item batch");
                                                }
                                                else
                                                {
                                                    #region Insert Detail Kartu Stok

                                                    //get stok akhir kartu stok detail
                                                    var cekKartuStokAkhirDetail = await this._kartuStokDao.GetStokAkhirDetailBatchWithLock(
                                                        batch.batch_number,
                                                        batch.expired_date,
                                                        item.id_stockroom,
                                                        detail.id_item
                                                    ...
                                                    var kartuStokDetailParam = new mm_kartu_stok_item_detail_batch_insert_pengurangan_stok
                                                    {
                                                        ...
                                                        id_detail_transaksi = batch.pemakaian_internal_detail_item_id,
                                                        id_header_transaksi = batch.pemakaian_internal_id,
                                                        nominal_awal = cekKartuStokAkhirDetail?.nominal_akhir,
                                                        stok_awal = cekKartuStokAkhirDetail?.stok_akhir,
                                                        nominal_keluar = batch.sub_total,
                                                        stok_keluar = batch.qty_pemakaian_internal
                                                    };
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            if ((cekStokItemDetail.qty_on_hand - batch.qty_pemakaian_internal) < 0)
                                                            {
                                                        ...
                                                        var stokItemDetailParam = new mm_setup_stok_item_detail_update_pengurangan_stok
                                                        {
                                                            ...
                                                            id_stockroom = item.id_stockroom,
                                                            qty_on_hand = batch.qty_pemakaian_internal,
                                                            batch_number = batch.batch_number,
                                                            expired_date = batch.expired_date
                                                        };
        ...
        public async Task<(bool, long, string)> UpdateTrPemakaianInternalCanceled(tr_pemakaian_internal_update_to_canceled data)
        {
        ...
        public async Task<List<tr_pemakaian_internal_detail_item_batch>> GetTrPemakaianInternalDetailItemBatchByDetailItemId(long pemakaian_internal_detail_item_id)
        {
            try
            {
                return await this._dao.GetTrPemakaianInternalDetailItemBatchByDetailItemId(pemakaian_internal_detail_item_id);
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<List<tr_pemakaian_internal_detail_item_batch>> GetTrPemakaianInternalDetailItemBatchByPemakaianInternalId(long pemakaian_internal_id)
        {
            try
            {
                return await this._dao.GetTrPemakaianInternalDetailItemBatchByPemakaianInternalId(pemakaian_internal_id);
            }
            catch (Exception)
            {
                throw;
