...
{
    public interface ITransPenerimaanNoEdService
    {
        #region header

        Task<List<tr_penerimaan_no_ed>> GetAllTrPenerimaanByParams(List<ParameterSearchModel> param);
        Task<tr_penerimaan_no_ed> GetTrPenerimaanById(long penerimaan_id);

        Task<(bool, string)> AddTrPenerimaan(tr_penerimaan_no_ed_insert data);

        Task<(bool, string)> UpdateToValidated(tr_penerimaan_no_ed_update_status_to_validated data);
        Task<short> UpdateToCanceled(tr_penerimaan_no_ed_update_status_to_canceled data);

        #endregion

        #region detail item

        Task<List<tr_penerimaan_no_ed_detail_item>> GetTrPenerimaanDetailItemByHeaderId(long penerimaan_id);

        #endregion

        #region detail upload

        Task<List<tr_penerimaan_no_ed_detail_upload>> GetTrPenerimaanDetailUploadByHeaderId(long penerimaan_id);
        Task<long> AddTrPenerimaanDetailUpload(tr_penerimaan_no_ed_detail_upload_insert data);
        Task<short> DeleteTrPenerimaanDetailUpload(long penerimaan_detail_upload_id);

        #endregion
    }

    public class TransPenerimaanNoEdService : ITransPenerimaanNoEdService
    {
        private readonly SQLConn _db;
        private readonly TransPenerimaanNoEdDao _dao;
        ...
        public TransPenerimaanNoEdService(
            SQLConn db,
            TransPenerimaanNoEdDao dao,
        ...
        public async Task<(bool, string)> AddTrPenerimaan(tr_penerimaan_no_ed_insert data)
        {
        ...
        public async Task<(bool, string)> UpdateToValidated(tr_penerimaan_no_ed_update_status_to_validated data)
        {
            this._db.beginTransaction();
            try
            {
                                ...
                                if (rekapForHeader.Count > 0)
                                {
                                    foreach (var item in rekapForHeader)
                                    {
                                        ...
                                        var kartuStokParam = new mm_kartu_stok_item_insert_penambahan_stok
                                        {
                                            ...
                                            keterangan = "TERIMA PEMBELIAN BARANG TANPA ED",
        ...
        public async Task<List<tr_penerimaan_no_ed_detail_item>> GetTrPenerimaanDetailItemByHeaderId(long penerimaan_id)
        {
            try
            {
                return await this._dao.GetTrPenerimaanDetailItemByHeaderId(penerimaan_id);
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<List<tr_penerimaan_no_ed_detail_upload>> GetTrPenerimaanDetailUploadByHeaderId(long penerimaan_id)
        {
            try
            {
                return await this._dao.GetTrPenerimaanDetailUploadByHeaderId(penerimaan_id);
            }
            catch (Exception)
            {
                throw;
